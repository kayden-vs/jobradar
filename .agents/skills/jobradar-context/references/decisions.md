...
## ADR-012: Migrate AI Scorer from Groq to Google Gemini 2.0 Flash

**Date**: 2026-07-06
**Decision**: Replace Groq (`meta-llama/llama-4-scout-17b-16e-instruct`) with Google Gemini (`gemini-2.0-flash`) as the AI scoring provider, and remove the per-run token budget ceiling.
...
**Rationale**:
- Gemini 2.0 Flash free tier: ~1M TPM / ~1.5M TPD — 33× more tokens per minute than Groq.
- RPM: 15 RPM (confirmed) — controlled via 4.5s inter-request interval.
...
**Why NOT gemini-2.5-flash**: Initially chosen but abandoned after first live run (v5). 2.5-flash free tier is only 5 RPM (not 10–15 as documented). Worse, 2.5-flash is a THINKING model — its chain-of-thought tokens contaminate `response_mime_type=application/json` output causing JSON parse errors on every response despite 200 OK status. 2.0-flash is non-thinking and has 3× higher RPM.
